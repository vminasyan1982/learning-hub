    ...
    <div className={styles.page}>
      ...
      <div className={pageStyles.cards}>
        <div className={pageStyles.card}>
          ...
          <div className={pageStyles.cardSub}>AMD</div>
        </div>
        <div className={pageStyles.card}>
          <div className={pageStyles.cardLabel}>Total Actual</div>
          <div className={pageStyles.cardValue} style={{ color: "var(--color-info)" }}>{data ? fmt(data.total_actual) : "—"}</div>
          <div className={pageStyles.cardSub}>AMD</div>
        </div>
        <div className={pageStyles.card}>
          ...
          <div className={pageStyles.cardSub}>{data && data.total_planned > 0 ? `${Math.round((1 - data.total_actual / data.total_planned) * 100)}% remaining` : ""}</div>
        </div>
      </div>
        ...
        <div className={pageStyles.chartCard}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data.by_quarter} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              ...
              <Tooltip formatter={(v: number) => fmt(v) + " AMD"} />
              <Legend />
